...
默认会写入 `~/.openclaw/openclaw.json`（或由 环境变量`OPENCLAW_STATE_DIR` 指定openclaw主目录，比如~/.openclaw）：
- `plugins.allow` 追加 `openclaw-cozeloop-trace`
- `plugins.entries["openclaw-cozeloop-trace"].enabled = true`
- `plugins.entries["openclaw-cozeloop-trace"].config.authorization`（自动补全为 `Bearer ...`）
